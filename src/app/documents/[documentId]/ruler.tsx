  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
      ...
      if (container) {
        ...
        if (isDraggingLeft) {
          const maxLeftPostion = PAGE_WIDTH - rightMargin - MINIMUM_SPACE;
          const newLeftPostion = Math.min(rawPosition, maxLeftPostion);
          setLeftMargin(newLeftPostion); //TODO: Make colaborative
        } else if (isDraggingRight) {
          const maxRightPostion = PAGE_WIDTH - (leftMargin + MINIMUM_SPACE);
          const newRightPostion = Math.max(PAGE_WIDTH - rawPosition, 0);
