...
const TableSizeSelector = ({
  ...
  onTableSelect: ({ rows, cols }: { rows: number; cols: number }) => void;
}) => {
  ...
  const handleCellClick = (row: number, col: number) => {
    onTableSelect({ rows: row + 1, cols: col + 1 });
  };
    ...
    for (let row = 0; row < maxRows; row++) {
      for (let col = 0; col < maxCols; col++) {
        ...
        cells.push(
          <div
            key={`${row}-${col}`}
            className={`w-4 h-4 border cursor-pointer transition-all duration-75 ${
              isHighlighted
              ...
              <MenubarMenu>
                ...
                <MenubarContent>
                  <MenubarSub>
                    ...
                    <MenubarSubContent>
                      <TableSizeSelector onTableSelect={insertTable} />
                    </MenubarSubContent>
                  </MenubarSub>
                </MenubarContent>
              </MenubarMenu>
